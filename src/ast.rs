    ...
    pub fn get_slot0_sqrt_price_x96(&self) -> Option<(BigInt, BigInt)> {
        ...
        let storage_change_opt = self.storage_changes.iter().find(|storage_change| {
            storage_change.address == Self::CONTRACT_ADDR
                && storage_change.key.eq(output.as_slice())
        });
        ...
        Some((
            BigInt::from_unsigned_bytes_be(old_data.as_slice()),
            BigInt::from_unsigned_bytes_be(new_data.as_slice()),
        ))
    }
